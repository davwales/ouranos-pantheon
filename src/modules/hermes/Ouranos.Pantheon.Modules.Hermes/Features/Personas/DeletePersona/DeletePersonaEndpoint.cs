...
public static class DeletePersonaEndpoint
{
    ...
    {
        app.MapDelete("/api/hermes/personas/{personaId}", Handle).WithTags("Hermes.Personas");
    }
    ...
    {
        return Results.Ok(
            await bus.InvokeAsync<DeletePersonaResponse>(new DeletePersonaInput(personaId), ct)
        );
    }
}
