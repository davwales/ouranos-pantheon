    ...
    : IPantheonHandler<GetAllStrategiesInput, PagedResponse<GetAllStrategiesResponse>>
{
    ...
    )
    {
        ...
        var strategies = await _dbContext
        ...
        var backtestSummaries = await _dbContext
            ...
            .Select(g => new
            {
                StrategyId = g.Key,
                Count = g.Count(),
                LastReturn = g.OrderByDescending(b => b.CreatedAt)
                    .Select(b => b.Results != null ? b.Results.TotalReturnPercent : (decimal?)null)
                    .FirstOrDefault(),
                LastWinRate = g.OrderByDescending(b => b.CreatedAt)
                    .Select(b => b.Results != null ? b.Results.WinRate : (decimal?)null)
                    .FirstOrDefault(),
            })
        ...
        return new PagedResponse<GetAllStrategiesResponse>(
            items,
            totalCount,
            input.Skip,
            input.Take
