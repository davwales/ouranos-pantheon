...
public sealed class GetSymbolGroupHandler
    : IPantheonHandler<GetSymbolGroupInput, GetSymbolGroupResponse>
{
    private readonly PlutusDbContext _dbContext;
    private readonly ILogger<GetSymbolGroupHandler> _logger;

    public GetSymbolGroupHandler(ILogger<GetSymbolGroupHandler> logger, PlutusDbContext dbContext)
    {
    ...
    )
    {
        ...
        var symbols = group
            .Members.Select(m =>
            {
                ...
                return new SymbolGroupSymbolResponse(
                    ...
                    Gain: foundSnapshot
                        ? (snap.MaxPrice - snap.MinPrice - snap.Tax)
                            * (snap.TotalVolume > snap.Limit ? snap.Limit : snap.TotalVolume)
                        : null,
        ...
        return new GetSymbolGroupResponse(
            group.Id,
            group.MarketId,
            group.Name,
            group.Description,
    ...
    private async Task<(
        Dictionary<
            Id<Symbol>,
            (decimal MinPrice, decimal MaxPrice, decimal TotalVolume, decimal Limit, decimal Tax)
        >,
