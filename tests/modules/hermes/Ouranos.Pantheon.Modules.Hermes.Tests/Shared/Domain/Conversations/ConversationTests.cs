...
public sealed class ConversationTests
{
    ...
    public void Create_WhenHappyPath_ShouldSetExpectedProperties()
    {
        ...
        // Act
        var conversation = Conversation.Create(id, personaId, modelConfigId, [], [], name, true);
    ...
    public void Create_WhenPersonaIdMismatch_ShouldThrowArgumentException()
    {
        ...
        var personaId = new Id<Persona>(Guid.NewGuid().ToString());
        var modelConfigId = new Id<ModelConfig>(Guid.NewGuid().ToString());
        var mismatchedPersona = Persona.Create(new Id<Persona>(Guid.NewGuid().ToString()), _fixture.Create<string>(), _fixture.Create<string>());
    ...
    public void Create_WhenModelConfigIdMismatch_ShouldThrowArgumentException()
    {
        ...
        var personaId = new Id<Persona>(Guid.NewGuid().ToString());
        var modelConfigId = new Id<ModelConfig>(Guid.NewGuid().ToString());
        var mismatchedModel = ModelConfig.Create(new Id<ModelConfig>(Guid.NewGuid().ToString()), _fixture.Create<string>(), _fixture.Create<string>(), _fixture.Create<string>());
    ...
    public void Update_WhenHappyPath_ShouldUpdateProperties()
    {
        // Arrange
        var id = new Id<Conversation>(Guid.NewGuid().ToString());
        var conversation = Conversation.Create(id, new Id<Persona>(Guid.NewGuid().ToString()), new Id<ModelConfig>(Guid.NewGuid().ToString()), [], []);
        ...
        var message = Message.Create(new Id<Message>(Guid.NewGuid().ToString()), id, _fixture.Create<string>(), Role.User, 0);
    ...
    public void Update_WhenInvalidName_ShouldThrowArgumentException(string? newName)
    {
        ...
        // Act
        var update = () => conversation.Update(newName!, new Id<Persona>(Guid.NewGuid().ToString()), new Id<ModelConfig>(Guid.NewGuid().ToString()), [], [], true);
