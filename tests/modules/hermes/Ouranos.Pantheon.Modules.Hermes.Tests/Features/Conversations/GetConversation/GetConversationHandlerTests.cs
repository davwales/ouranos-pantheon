...
public sealed class GetConversationHandlerTests
{
    ...
    public async Task Handle_WhenHappyPath_ShouldReturnFullConversationResponse()
    {
        ...
        var modelConfig = ModelConfig.Create(new Id<ModelConfig>(Guid.NewGuid().ToString()), _fixture.Create<string>(), _fixture.Create<string>(), _fixture.Create<string>());
