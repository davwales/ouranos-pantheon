...
public sealed class SymbolSignalCalculateJobTests
{
    ...
    [Fact]
    public async Task Execute_WhenExistingSignals_ShouldPreserveInsteadOfDeleting()
    {
        // Arrange
        var market = Market.Create(
            new Id<Market>(Guid.NewGuid().ToString()),
        ...
        var existingSignal = Signal.Create(market.Id, symbol.Id, SignalType.TaxAdjustedRoi, 0.1m);

        var computer = Substitute.For<ISignalComputer>();
        computer.Type.Returns(SignalType.Rsi);
        computer
            .ComputeAsync(Arg.Any<SignalComputeContext>(), Arg.Any<CancellationToken>())
            .Returns(Task.FromResult<decimal?>(0.5m));

        var job = CreateJob([computer]);

        await _dbContext.SeedData(market);
        await _dbContext.SeedData(symbol);
        await _dbContext.SeedData(existingSignal);
